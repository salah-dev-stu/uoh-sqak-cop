...
- **Tamper sweep** — not "a tampered record is caught" but "**all N mutations of the real committed log are caught, N = 1811 today, 0 escapes**" — a one-line Integrity proof no prose can match.
...
**IC-3 — Count formula and floor.** Expected total `N = Σ_i (9 + D_i) = 10·R + Σ_i (D_i) − R` … stated exactly: `N = R·10 + Σ_i D_i` where `R = len(records)` and `D_i = |{distinct hex digits in commit_i}|` (rows a–e give 9 payload mutations, row f gives 1, row g gives `D_i`). For the committed log: `R = 70`, `Σ D_i = 1111` → **`N = 700 + 1111 = 1811`**. The test asserts `N ≥ 500` (PLAN-PHENOMENAL §2 P8 acceptance floor) so a regenerated shorter sample run still satisfies the gate.
...
| # | Check |
|---|---|
| A1 | Tamper sweep: **≥ 500 mutations, 0 escapes** (current log: 1811/1811), each localised to its record index; pristine log passes (IC-1..4) |
