...
| **Depends on** | `domain/crypto.py` (`CommitReveal`, `audit_records`) · `domain/canonical.py` · `domain/belief.py` · `domain/board.py` · `domain/protocol.py` · `peer/turn_handler.py` · `gui/replay_data.py` · `tests/fakes/fake_transport.py` (`.sent` tap) · committed sample log `docs/sample-run/log_uoh-sqak-police-729eaa25_g01.json` |
