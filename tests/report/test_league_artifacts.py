...
def test_the_result_declares_the_league_counters_truthfully() -> None:
    ...
    # Earned by WINNING a counted first meeting. We used to award it to both
    # teams, which put a false True in the loser's column of every filed
    # counted result — invisible in friendlies, where nobody earns it.
    winner = league_fields["winner_group"]
    assert league_fields["diversity_reward_applied"] == {
    ...
    assert again["final_result"]["diversity_reward_applied"] == dict.fromkeys(
        ("uoh-sqak", "imreeyal"), False)  # a rematch is not a new opponent
